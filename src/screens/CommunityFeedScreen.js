...
export default function CommunityFeedScreen({ navigation }) {
  ...
  const categories = ['All', 'General', 'Testimony', 'Prayer', 'Encouragement'];
  ...
  const uploadImage = async (uri) => {
    try {
      ...
      const fileName = `communityFeed/${user.uid}/${Date.now()}.jpg`;
  ...
  const handleLike = async (post) => {
    try {
      ...
      if (isLiked) {
        ...
        const willBeFirstLike = currentLikesCount === 0;
        ...
        if (post.userId !== user.uid && willBeFirstLike) {
          try {
            // Create post object with correct likesCount for notification
            const postForNotification = {
              id: post.id,
              userId: post.userId,
              userName: post.userName,
              likesCount: 1, // This is the first like
            };
            ...
            if (__DEV__) {
              if (notificationResult.success) {
                console.log('✅ Like notification sent successfully:', notificationResult);
              } else {
        ...
        } else if (__DEV__ && post.userId !== user.uid) {
          console.log(`ℹ️ Skipping like notification - not first like (current count: ${currentLikesCount})`);
        }
      }
    } catch (error) {
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, 'communityFeed', selectedPost.id, 'comments'), commentData);

      // Update comment count
      await updateDoc(doc(db, 'communityFeed', selectedPost.id), {
      ...
      if (selectedPost.userId !== user.uid) {
        try {
          ...
          if (__DEV__) {
            if (notificationResult.success) {
              console.log('✅ Comment notification sent successfully:', notificationResult);
            } else {
              console.warn('⚠️ Comment notification failed:', notificationResult.error);
              if (notificationResult.skippedUsers > 0) {
                console.warn(`   - ${notificationResult.skippedUsers} user(s) skipped due to preferences`);
              }
              if (notificationResult.tokenCount === 0) {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteDoc(doc(db, 'communityFeed', post.id));
  ...
  const getCategoryIcon = (category) => {
    const icons = {
      ...
      Encouragement: 'bulb-outline',
    };
    return icons[category] || 'chatbubbles-outline';
  };
      ...
      {loading && !refreshing ? (
        ...
        <View style={styles.centerContainer}>
          ...
          >
            ...
            <Text style={styles.emptyActionButtonText}>Create Post</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <ScrollView
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody}>
              ...
              <View style={styles.categorySelect}>
                {['General', 'Testimony', 'Prayer', 'Encouragement'].map((cat) => (
                  <TouchableOpacity
                    key={cat}
                    style={[
              ...
              <TextInput
                ...
                placeholder="Share your thoughts, testimony, or prayer request... Use #hashtags to tag your post"
                value={postContent}
